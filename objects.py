...
class Player2:
	...
	def update(self, pressed_keys, game_over, level_won, game_won):
		...
		if not game_over and not game_won:
			if (pressed_keys[K_w] or pressed_keys[K_SPACE]) and not self.jumped and not self.in_air:
				self.vel_y = -15
				jump_fx.play()
				self.jumped = True
			if (pressed_keys[K_w] or pressed_keys[K_SPACE]) == False:
				self.jumped = False
			if pressed_keys[K_a]:
				dx -= 5
				self.counter += 1
			...
			if pressed_keys[K_a] == False and pressed_keys[K_d] == False:
				self.counter = 0
				self.index = 0
				self.image = self.img_right[self.index]
